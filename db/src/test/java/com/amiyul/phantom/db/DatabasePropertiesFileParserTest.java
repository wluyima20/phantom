...
public class DatabasePropertiesFileParserTest {
	...
	public void createInstance_shouldCreateADbConfigMetadataInstance() throws Exception {
		...
		assertNull(metadata.getDatabaseDefinitions().get(0).getStatus().getUnavailableUntil());
		...
		assertEquals(expectedDate, metadata.getDatabaseDefinitions().get(1).getStatus().getUnavailableUntil());
	}
